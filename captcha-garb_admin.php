...
function captcha_garb_admin_option() {
  add_options_page('Captcha Garb Settings','Captcha Garb', 7,__FILE__, 'captcha_garb_admin_page');
}



function captcha_garb_admin_page() {  if(!is_admin()) { echo '<div class="error"><p>LOl What are you doing here ? =)).</p></div>'; return false; }  ?>
...
<td>
<label>
<textarea style="width:100%;height:200px"><?php echo get_option("cgarb_css"); ?></textarea>
</label>
</td>
</tr>
...
<label>Upload New Image :</label><input type="file" name="cgarb_new_image"/><input type="submit" name="Upload"/>
